...
class PostEditScreen extends StatefulWidget {
  ...
  const PostEditScreen({super.key, this.post, this.fundName, required this.isEditing});
...
class PostEditScreenState extends State<PostEditScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    fundNameController.text = widget.fundName ?? widget.post?.fundName ?? "";
    print(widget.fundName);
    HtmlEditor editor = HtmlEditor(
      controller: bodyController,
      callbacks: Callbacks(
        onInit: () {
          bodyController.setText(widget.post?.body ?? "");
        }
      ),
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Container(
                    margin: const EdgeInsets.fromLTRB(0, 9, 0, 0),
                    child: const Text("구분 (조합명)", style: headingStyle)
                ),
                ...
                Column(
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: TextField(
                        ...
                        onEditingComplete: () async {
                          setState(() async {
                            if (fundNameController.text.isNotEmpty) {
                              isValidFundName = await isExistingFundName(fundNameController.text);
                            } else {
                              isValidFundName = true;
                            }
                          });
                        },
                        style: const TextStyle(
                        ...
                        decoration: const InputDecoration(
                          ...
                          hintText: "조합명을 입력하세요. 입력하지 않으면 전체 공지가 됩니다.",
                          border: OutlineInputBorder(
                              borderSide: BorderSide(color: Color(0xffdddddd)),
                              borderRadius: BorderRadius.zero
                          ),
                    ...
                    if (!isValidFundName) Container(
                      margin: const EdgeInsets.fromLTRB(0, 10, 0, 0),
                      child: WidgetUtils.errorMessage("존재하지 않는 조합명입니다."),
                    )
                  ],
                ),
              ],
            ...
            Container(
              ...
              child: WidgetUtils().buttonBar("목록", "저장", 
                ...
                () async {
                  print(await bodyController.getText());
                  print(titleController.text);
                  if (widget.isEditing) {
                    var response = await editPost(
                        widget.post!.id,
                        fundNameController.text.isEmpty? null : fundNameController.text,
